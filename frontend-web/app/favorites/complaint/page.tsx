...
function UserComplaintSubmissionContent() {
  const { user } = useUser();
  const router = useRouter();
  const searchParams = useSearchParams();

  // Read pharmacy context from URL params (set when coming from a specific pharmacy page)
  const pharmacyParam = searchParams.get("pharmacy") ?? "";
  const pharmacyIdParam = searchParams.get("pharmacy_id") ?? "";
  // When locked, the pharmacy name is fixed and history is filtered to that pharmacy
  const isPharmacyLocked = !!pharmacyParam;
  ...
  // Form states
  const [pharmacyName, setPharmacyName] = useState(pharmacyParam);
  ...
  // Sync pharmacy name if URL param changes
  useEffect(() => {
    if (pharmacyParam) {
      setPharmacyName(pharmacyParam);
    }
  }, [pharmacyParam]);

  // Filter history: when locked to a specific pharmacy, show only that pharmacy's complaints
  const displayedComplaints = isPharmacyLocked
    ? complaints.filter((c) =>
        c.pharmacy_name?.toLowerCase().trim() === pharmacyParam.toLowerCase().trim()
      )
    : complaints;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include pharmacy_id when coming from a specific pharmacy page
      if (pharmacyIdParam) {
        payload.pharmacy_id = pharmacyIdParam;
      }
      ...
      // Keep pharmacy locked if coming from pharmacy page; otherwise clear it
      if (!isPharmacyLocked) setPharmacyName("");
      setComplaintSummary("");
      ...
      if (err.details && Array.isArray(err.details)) {
        const errorList = err.details
          ...
          .join(" | ");
        setSubmitError(`Validation failed - ${errorList}`);
      } else {
  ...
  const statusStyles: Record<string, { bg: string; dot: string }> = {
    New: { bg: "bg-red-50 text-red-600 border-red-100", dot: "bg-red-505" },
    ...
    <div className="flex-1 bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] p-4 sm:p-6 pt-24 sm:pt-28 space-y-6 overflow-y-auto min-h-screen relative isolate font-sans">
      ...
      <div className="relative z-10 max-w-5xl mx-auto">
        {isPharmacyLocked && (
          <button
            onClick={() => router.back()}
            className="inline-flex items-center gap-2 text-slate-500 hover:text-slate-800 font-bold text-sm mb-4 transition-colors"
          >
            ← Back to Pharmacy
          </button>
        )}
        <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
          {isPharmacyLocked ? `Report Issue — ${pharmacyParam}` : "Report an Issue"}
        </h1>
        <p className="text-sm font-medium text-slate-500 mt-1">
          {isPharmacyLocked
            ? `Your report will be sent privately to admins only. The pharmacy cannot see it.`
            : "Encountered an issue with a pharmacy listing? Tell our administrators immediately."}
        </p>
        {isPharmacyLocked && (
          <div className="mt-3 inline-flex items-center gap-2 bg-rose-50 border border-rose-100 text-rose-700 px-4 py-2 rounded-xl text-xs font-bold">
            🔒 This complaint is private — visible only to you and admins
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  {isPharmacyLocked ? (
                    <div className="w-full px-3.5 py-2.5 border border-rose-200 bg-rose-50/40 rounded-xl text-xs font-bold text-rose-800 flex items-center gap-2">
                      🔒 {pharmacyName}
                    </div>
                  ) : (
                    <input
          ...
          <div className="bg-white rounded-2xl border border-white shadow-md p-4 space-y-3">
            <div className="flex items-center gap-2 pb-2 border-b border-slate-100">
              ...
              <h2 className="text-xs font-black uppercase tracking-wider text-slate-700">
                {isPharmacyLocked ? `Complaints for ${pharmacyParam}` : "Your Dispatched Tickets"}
              </h2>
            </div>
            {isPharmacyLocked && (
              <p className="text-[10px] text-slate-400 font-medium pb-1">
                Showing only your complaints against this pharmacy. Admins can see all complaints.
              </p>
            )}
            ...
            {loadingHistory ? (
              ...
              <p className="text-center text-[11px] text-slate-400 font-medium py-8 italic">
                {isPharmacyLocked ? `No complaints filed against ${pharmacyParam} yet.` : "No complaints submitted yet."}
              </p>
            ) : (
...
export default function UserComplaintSubmission() {
  return (
    <Suspense fallback={
      ...
      <UserComplaintSubmissionContent />
    </Suspense>
  );
}
